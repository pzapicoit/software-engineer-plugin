...
1. **Identificar tarea activa** — lee `.intermarkit/task-metrics/.active`. Obten `issue_key` y, si existe, el campo `repos` (lista de `name`) del JSON asociado.
...
3. **Commit final + push** — si el fichero de metricas trae `repos` (multi-repo), repite estos pasos para CADA repo de la lista usando su `path` (consulta `.intermarkit/config.yaml` para resolver `name` -> `path`/`url`/`workspace`). Si no hay `repos` (proyecto de un solo repo), ejecuta una vez en la raiz:
   ```bash
   git -C "{path}" status
...
4. **Crear PR(s)** — `bitbucketPullRequest` (create), uno por cada repo que haya recibido push en el paso 3:
   ...
   - Usa el `workspace`/repositorio propio de ese repo (nunca el de otro repo de la lista).
...
8. **Comentario Jira** — `addCommentToJiraIssue` con la plantilla de `agents/reference.md` §Plantilla de comentario Jira. Incluye tiempo, tool calls, tokens (formato M/K + cache hit %), total de tokens y coste estimado en € (prefijo `≈`). Si `repos` tiene mas de un elemento, usa el bloque `**PRs:**` con un enlace por repo (ver plantilla); si es un solo repo, usa la linea singular `**PR:**`. Anade la linea de `context peak` solo si el fichero la trae. No inventes cifras: si `tokens.turns == 0`, omite esas lineas.
...
10. **Confirmar cierre** al usuario:
    - Rama(s) pusheada(s) + PR(s) creado(s) (o pendiente), indicando el repo de cada uno si es multi-repo
    - Issue en "In Testing"
